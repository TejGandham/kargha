...
Loaded by `kargha-build` only when a ticket touches GitHub Actions, CI/CD, repository automation, branch policy, rulesets, required checks, deployment policy, generated contracts, or environment policy.
...
Normal `kargha-build` flow should not use admin bypass. If the user explicitly authorizes break-glass, report:

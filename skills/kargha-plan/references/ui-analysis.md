...
Loaded by `kargha-plan` **only when the resolved stack has a UI surface** — a design mock or non-functional prototype as input, or a component-library / token system in the repo. Non-UI stacks (backend, CLI, data, library/SDK, IaC, ML, docs) skip this file entirely.

This is kargha's frontend analysis at full depth. It is the conditional UI path layered on top of the lean binder workflow — it does **not** replace the stack-agnostic synthesis. Its job is to feed the binder, not to write tickets:

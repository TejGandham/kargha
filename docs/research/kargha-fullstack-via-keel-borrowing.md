...
- **T1 — stays a skill.** The line is *persisted inter-run state + multi-agent scheduling*, not prompt length or subagent count. Full-stack does not cross it — but pushes it to the edge (see depends_on correction).
...
- **T4/T6 — 4 skills:** `plan` (extended + a plan-time coherence-review subagent), `build` (layer-dispatcher), **`verify` (NEW** read-only behavioral/safety gate; behavioral analog of `validate`; auto-invoked by build), `validate` (unchanged, visual). The gate must be independent of the implementer.
- **T5 — pairwise coherence suffices, with two additions.** A **project Domain Invariants registry** gives global rule enforcement without a Binder (*the invariant is the global mechanism; the Binder was just a delivery vehicle*). Gaps: plan-time transitive consistency → a coherence-review subagent; globally-ordered migrations → below.
...
- **Plan-time coherence** — a read-only review subagent in kargha-plan (the only point all sibling tickets are visible) catches transitive schema/auth inconsistencies.

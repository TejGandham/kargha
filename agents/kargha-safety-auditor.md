...
The attempt counter is the orchestrator's; you store no loop state. Unlike `kargha-acceptance-reviewer` (which halts with a call-to-action at 2 and never reaches a human), this gate **does** escalate to the human after 3 — an unjustified boundary crossing is a safety question that a person must adjudicate.
...
```
## Kargha Boundary Scan: [work item title]
...
routing_hints:
  next: null
  kickback_to: kargha-build | null    # set on VIOLATION
